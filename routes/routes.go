...
func Setup(app *fiber.App, db *mongo.Database) {
	...
	usersGroup.Delete("/", controllers.DeleteUsers)             // Xoá user, chỉ admin được phép
	// === Product routes ===
	...
	invoices.Get("/", invoiceController.FilterByDate)  // GET /api/invoices?from=dd/mm/yyyy&to=dd/mm/yyyy&page=1&limit=10&deleted=false&shift=morning -> lọc hóa đơn
